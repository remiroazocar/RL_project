...
model_dir = "../models/A6i" # directory for saving models
...
result_container = np.zeros([episode_no, 2]) # container for performance of different models
# for greedy policy
greedy_eps = 0.05

# weights
w = {'L1': tf.Variable(tf.truncated_normal([4, sizehidden], 0, 0.015)),
     'L2': tf.Variable(tf.truncated_normal([sizehidden, 2], 0, 0.015))}
...
train = tf.train.RMSPropOptimizer(learning_rate=learningrate, momentum=rmsprop_mom, decay=rmsprop_dec).minimize(loss)
...
"""
training procedure here (TWO training setups either A6trainI or A6 train2)
...
#### MODEL II (experiment)
# load/saving functionality (for models)
model_dir = "../models/A6ii(experiment)" # directory for saving models
# if directory does not exist, create
if not os.path.exists(model_dir):
    os.makedirs(model_dir)
filename = '/A6'
...
perf = np.load('A6finalresults.npy')
print("Saving plots")
...
plt.savefig('A6_performance.png', bbox_inches='tight')
...
plt.savefig('A6_bellmanloss.png', bbox_inches='tight')
"""
