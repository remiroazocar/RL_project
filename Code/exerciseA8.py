...
model_dir = "../models/A8" # directory for saving models
...
with tf.Session() as sess:
    ...
    for epi in range(episode_no):
        ...
        # SARSA
        #
        feeddict = {currentstate: observation.reshape([1, 4])}
        actionb = sess.run(predict, feed_dict=feed_dict)
        ...
        while k < episode_no- 1:
            ...
            # SARSA UPDATE
            prevaction = actionb
            # modify reward according to rules
            if done:
                reward = -1
            # forward pass through NN gives action
            actionc = sess.run(prediction, feed_dict={currentstate: prev_obs.reshape(-1, 4)})
            # greedy policy w epsilon
            if np.random.uniform(0,1) > greedy_eps:
                actionc = actionc
            else:
                actionc = [(np.random.uniform(0,1) > 0.5) * 1]
    ...
    savingpath = saver.save(sess, "../models/A8/A8")
    # saved model
    print('Model saved.')
    i = i+1
"""
